defmodule CoffeeTime.Measurement.OneWireSensor do
  @moduledoc """
  ...
  def handle_info(:query, state) do
    ...
    temp =
      CoffeeTime.Hardware.read_one_wire_temperature(state.context.hardware, state.name)

    CoffeeTime.Measurement.Store.put(state.context, state.name, temp)
