defmodule CoffeeTime.Boiler.DutyCycle do
  @moduledoc """
  ...
  def set(context, int) when int in 0..10 do
    ...
    CoffeeTime.PubSub.broadcast(context, :boiler_duty_cycle, int)
  ...
  def init(%{context: context, intervals: %{__MODULE__ => %{write_interval: interval}}}) do
    ...
    {:ok, gpio} = CoffeeTime.Hardware.open_gpio(context.hardware, :duty_cycle)
  ...
  def terminate(reason, %{context: context, gpio: gpio}) do
    CoffeeTime.Hardware.write_gpio(context.hardware, gpio, 0)
    {:stop, reason}
  end
  ...
  def handle_continue(:cycle, state) do
    ...
    CoffeeTime.Hardware.write_gpio(state.context.hardware, state.gpio, gpio_val)
    {:noreply, state}
  end
