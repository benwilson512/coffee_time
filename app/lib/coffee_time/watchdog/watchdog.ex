defmodule CoffeeTime.Watchdog do
  @moduledoc """
  ...
  def init(%{context: context, config: config}) do
    ...
    state =
      if !state.fault do
        CoffeeTime.PubSub.subscribe(context, "*")
    ...
    if state.fault do
      CoffeeTime.PubSub.broadcast(context, :watchdog, :fault_state)
    else
      CoffeeTime.PubSub.broadcast(context, :watchdog, :ready)
    end
